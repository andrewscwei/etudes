...
describe('useTimeout', () => {
  ...
  it('invokes the handler after the specified timeout', () => {
    const handler = vi.fn()
    renderHook(() => useTimeout(100, handler))
  ...
  it('does not auto-start when shouldAutoStart is false', () => {
    const handler = vi.fn()
    renderHook(() => useTimeout(100, handler, { shouldAutoStart: false }))
  ...
  it('can be manually started', () => {
    const handler = vi.fn()
    const { result } = renderHook(() => useTimeout(100, handler, { shouldAutoStart: false }))
  ...
  it('can be stopped before firing', () => {
    const handler = vi.fn()
    const { result } = renderHook(() => useTimeout(100, handler))
  ...
  it('stops on unmount', () => {
    const handler = vi.fn()
    const { unmount } = renderHook(() => useTimeout(100, handler))
  ...
  it('does not start when timeout is negative', () => {
    const handler = vi.fn()
    renderHook(() => useTimeout(-1, handler))
